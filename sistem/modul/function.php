...
/*
 * Tambahan parameter hargaBarang untuk ahadPOS UKM
 */

function tambahBarangJual($barcode, $jumBarang, $hargaBarang = NULL) {
...
//      if($jumBarang==0){
        ...
        /*
         * Jika tidak ada $hargaBarang maka dipakai harga jual asli
         */
        if (is_null($hargaBarang)){
        ...
        $sql = "INSERT into tmp_detail_jual(idCustomer, tglTransaksi,
                        ...
                        VALUES('$_SESSION[idCustomer]','$tgl','$barcode',
                            '$jumlah','$hargaBeli','$hargaBarang','$_SESSION[uname]', $idBarang)"; // harga Barang (ahadPOS UKM)

        mysql_query($sql) or die(mysql_error());
        $uid = mysql_insert_id();
        if ($uid) {
            /*
             * cek diskon admin terlebih dahulu (ahadPOS UKM)
             */
            cekDiskonAdmin($uid);
